...
$(document).ready(function() {
    $('#formPlantilla').on('submit', function(e) {
        ...
        btn.prop('disabled', true).html(
            '<span class="glyphicon glyphicon-refresh"></span> Procesando...');
        ...
        $.ajax({
            ...
            complete: function() {
                if ($('#plan_id_edit').val() == 0) {
                    ...
                    btn.prop('disabled', false).html(
                        '<span class="glyphicon glyphicon-refresh"></span> Actualizar Plantilla'
                        );
                }
            }
        });
...
function editarPlantilla(id) {
    ...
    $('#btnGuardar').removeClass('btn-success').addClass('btn-warning').html(
        '<span class="glyphicon glyphicon-refresh"></span> Actualizar Plantilla');
    $('#btnCancelar').show();

    $('html, body').animate({
        scrollTop: $('#formPlantilla').offset().top - 20
    }, 'fast');
}
...
function cancelarEdicion() {
    ...
    $('#btnGuardar').removeClass('btn-warning').addClass('btn-success').html(
        '<span class="glyphicon glyphicon-floppy-disk"></span> Guardar Plantilla');
    $('#btnCancelar').hide();
}
