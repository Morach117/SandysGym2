...
$dir_hero = $root_dir . '/assets/img/hero/';
$dir_galeria = $root_dir . '/assets/img/gallery/';
...
if ($accion === 'nuevo_hero') {
    ...
    if(mysqli_query($conexion, "INSERT INTO san_landing_hero (subtitulo, titulo_html, imagen_bg) VALUES ('$subtitulo', '$titulo_html', '$img_name')")){
...
if ($accion === 'nuevo_plan') {
    ...
    // Purga array de beneficios vacíos y convierte a JSON
    ...
    if(mysqli_query($conexion, "INSERT INTO san_landing_planes (nombre, precio, frecuencia, beneficios_json, url_boton) VALUES ('$nombre', $precio, '$frecuencia', '$json', '$url_boton')")){
        echo json_encode(['exito' => true]);
    } else { echo json_encode(['exito' => false, 'mensaje' => mysqli_error($conexion)]); }
    exit;
}
...
if ($accion === 'nueva_galeria') {
    ...
    if(mysqli_query($conexion, "INSERT INTO san_landing_galeria (imagen_url, es_wide) VALUES ('$img_name', $es_wide)")){
        echo json_encode(['exito' => true]);
    } else { echo json_encode(['exito' => false, 'mensaje' => mysqli_error($conexion)]); }
    exit;
}
