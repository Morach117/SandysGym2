...
<form role="form" method="post" action=".?s=promociones&i=nuevo">
    ...
    <div class="col-md-4">
    <input type="number" class="form-control" name="cantidad_codigos" min="1" max="100" value="<?= ($tipo_promocion == 'Individual') ? (empty($cantidad_codigos) ? '5' : $cantidad_codigos) : $cantidad_codigos ?>" />
    </div>
</div>
...
<script>
    function toggleCantidadCodigos(select) {
        ...
        var cantidadCodigosInput = document.querySelector('input[name="cantidad_codigos"]');

        if (select.value === 'Individual') {
            cantidadCodigosRow.style.display = '';
            cantidadCodigosInput.value = '5';  // Asigna el valor 5 cuando se selecciona "Individual"
        } else {
            cantidadCodigosRow.style.display = 'none';
            cantidadCodigosInput.value = '';  // Limpia el valor si se selecciona otra opción
        }
    }
</script>
