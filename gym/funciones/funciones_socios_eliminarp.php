...
function eliminar_pago_socio($id_socio, $id_pago)
{
	...
	// INICIO DE TRANSACCIÓN: Protegemos la integridad del dinero
	mysqli_autocommit($conexion, false);

	try {
		// --- 1. LÓGICA DE MONEDERO: Extraemos el ID del abono desde san_pagos ---
		// AQUÍ SÍ USAMOS pag_id_prepago_abono PORQUE ESTAMOS LEYENDO san_pagos
		...
		if ($fila_pago = mysqli_fetch_assoc($res_pago)) {
			...
			// Si existe el abono, procedemos a restarlo
// Si existe el abono, procedemos a restarlo
			if ($id_abono > 0) {

				// --- CORRECCIÓN AQUÍ: Usamos pred_id_pdetalle tal como está en tu BD ---
				...
				if ($fila_monto = mysqli_fetch_assoc($res_monto)) {
					...
					// Restar el dinero del saldo total del socio
					...
					// Eliminar el registro del abono para limpiar el historial
		...
		if ($resultado) {
			...
			throw new Exception("Error en la consulta al eliminar el pago. " . mysqli_error($conexion));
		}

	} catch (Exception $e) {
		// Si cualquier paso falla (ej. error de SQL), deshacemos todo
