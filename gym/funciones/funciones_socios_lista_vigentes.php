<?php
	function lista_socios_fechas( $pag_busqueda )
	{
		...
		$parametros	= "&item=lista_vigentes";
		...
		$query		= "	SELECT		soc_id_socio AS id_socio
						...
						AND			DATE_FORMAT( pag_fecha_fin, '%Y-%m-%d' ) >= DATE_FORMAT( NOW(), '%Y-%m-%d' )
						AND			pag_status = 'A'
