...
function obtener_rutina_ejercicios_admin($filtro_nivel = null, $filtro_genero = null)
{
    ...
    try {
        ...
        if (mysqli_num_rows($resultado) > 0) {
            ...
            $datosHtml = "<tr><td colspan='10' class='text-center'>No se encontraron rutinas asignadas.</td></tr>";
        }
...
<head>
    ...
    <style>
        /* --- Estilos Generales --- */
        .container-fluid { width: 100%; padding: 15px; margin: auto; box-sizing: border-box; }
        ...
        .col-md-3, .col-md-4, .col-md-5, .col-md-6, .col-md-8, .col-md-9, .col-md-12 {
            position: relative; width: 100%; padding: 0 15px; box-sizing: border-box;
        }
        ...
        /* --- Título y Botón --- */
        .main-title { color: #333; font-weight: bold; font-size: 1.75rem; margin-bottom: 0; }
        .main-title i { margin-right: 10px; color: #17a2b8; }

        .btn {
            display: inline-block; font-weight: 400; color: #fff; text-align: center;
            vertical-align: middle; cursor: pointer; user-select: none;
            background-color: transparent; border: 1px solid transparent;
            padding: 0.375rem 0.75rem; font-size: 0.9rem; line-height: 1.5;
            border-radius: 0.25rem; transition: all .15s ease-in-out;
        }
        ...
        .btn-sm { padding: .25rem .5rem; font-size: .8rem; line-height: 1.5; border-radius: .2rem; }

        /* --- Card y Tabla --- */
        .card {
            background-color: #fff; border: 1px solid #e3e6f0; border-radius: 0.35rem;
            margin-top: 1.5rem; box-shadow: 0 0.15rem 1.75rem 0 rgba(58, 59, 69, 0.15);
        }
        .card-header {
            padding: 0.75rem 1.25rem; margin-bottom: 0; background-color: #f8f9fc;
            border-bottom: 1px solid #e3e6f0; font-weight: bold; color: #5a5c69;
        }
        .card-body { padding: 1.25rem; }
        .table-responsive { display: block; width: 100%; overflow-x: auto; }

        #tabla_rutinas_admin { width: 100%; margin-bottom: 1rem; color: #333; }
        #tabla_rutinas_admin th, #tabla_rutinas_admin td { padding: 0.75rem; vertical-align: middle; border-top: 1px solid #e3e3e3; }
        #tabla_rutinas_admin thead th { vertical-align: bottom; border-bottom: 2px solid #e3e3e3; background-color: #f8f9fc; color: #5a5c69; text-align: left; white-space: nowrap; }
        #tabla_rutinas_admin tbody tr:hover { background-color: #f1f1f1; }
        #tabla_rutinas_admin td.acciones { white-space: nowrap; text-align: right; }

        /* DataTables */
        .dataTables_wrapper .dataTables_length, .dataTables_wrapper .dataTables_filter { margin-bottom: 1rem; }
        .dataTables_wrapper .dataTables_filter input, .dataTables_wrapper .dataTables_length select {
            border-radius: .25rem; border: 1px solid #ced4da; padding: .375rem .75rem; margin-left: 0.5em; display: inline-block; width: auto;
        }
        .dataTables_wrapper .dataTables_paginate .paginate_button {
            padding: 0.5em 1em; margin-left: 2px; border: 1px solid #dee2e6; border-radius: .25rem; color: #007bff; cursor: pointer; background: white;
        }
        .dataTables_wrapper .dataTables_paginate .paginate_button.current, .dataTables_wrapper .dataTables_paginate .paginate_button:hover {
            color: #fff; background-color: #007bff; border-color: #007bff; text-decoration: none;
        }
        .dataTables_wrapper .dataTables_paginate .paginate_button.disabled, .dataTables_wrapper .dataTables_paginate .paginate_button.disabled:hover {
            color: #6c757d; background-color: white; border-color: #dee2e6; cursor: default;
        }

        /* --- Formulario Filtros --- */
        .form-filters label { font-weight: bold; margin-bottom: 0.5rem; display: block; }
        .form-filters select {
            display: block; width: 100%; padding: 0.375rem 0.75rem; font-size: 1rem;
            line-height: 1.5; color: #495057; background-color: #fff; border: 1px solid #ced4da;
        ...
        .modal-content {
            ...
            width: 80%; max-width: 700px; border-radius: 0.3rem; box-shadow: 0 5px 15px rgba(0, 0, 0, .5);
            position: relative; display: flex; flex-direction: column;
        }
        .modal-header {
            padding: 1rem 1rem; border-bottom: 1px solid #dee2e6; background-color: #f8f9fa;
        ...
        .close-button {
            float: right; font-size: 1.5rem; font-weight: 700; line-height: 1; color: #000;
            text-shadow: 0 1px 0 #fff; opacity: .5; cursor: pointer; background: transparent;
            border: 0; padding: 1rem 1rem; margin: -1rem -1rem -1rem auto;
        }
        .close-button:hover { opacity: .75; text-decoration: none; }
        .modal-body { position: relative; flex: 1 1 auto; padding: 1rem; }
        .modal-footer {
            display: flex; flex-wrap: wrap; align-items: center; justify-content: flex-end;
            padding: 0.75rem; border-top: 1px solid #dee2e6; border-bottom-right-radius: calc(.3rem - 1px);
            border-bottom-left-radius: calc(.3rem - 1px);
        }
        .modal-footer > * { margin: 0.25rem; }

        /* Formulario Modal */
        #formAsignacion .form-group { margin-bottom: 1rem; }
        #formAsignacion label { font-weight: bold; margin-bottom: 0.5rem; display: block; }
        #formAsignacion select, #formAsignacion input[type="number"], #formAsignacion input[type="text"] {
            display: block; width: 100%; padding: 0.375rem 0.75rem; font-size: 1rem;
        ...
        #formAsignacion input:focus, #formAsignacion select:focus {
