<?php
	...
	function obtener_lista_para_entrega( $p_order_by, $p_credito, $p_buscar = '', $p_status = '' )
	{
		...
		if( $p_credito == 1 )
			$condicion	= " AND ven_total_credito > 0 "; //Con pago pendiente
		else
			$condicion = " AND ven_total_credito <= 0 "; //Notas pagadas
		...
		if( $p_status == 'L' )
			$condicion .= " AND ( ven_status = 'L' OR ( ven_status = 'E' AND ven_total_credito > 0 ) )"; //lav - entregado
		elseif( $p_status == 'S' )
			$condicion .= " AND ( ven_status = 'S' OR ( ven_status = 'T' AND ven_total_credito > 0 ) )"; //planch entregado
		...
		$query		= "	SELECT		ven_id_venta AS id_venta,
						...
						WHERE		ven_status NOT IN ( 'C', 'I', 'P', 'R' )
