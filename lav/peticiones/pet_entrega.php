<?php
	...
	//rol = S = superusuario - tambien al guardar se valida el rol
	if( $rol == 'S' )
		$tag_rol = "";
	else
		$tag_rol = "readonly disabled";
	
	$cmb_dias	= combo_dias( date( 'd' ) );
	$cmb_meses	= combo_meses( date( 'm' ) );
	$cmb_años	= combo_años( date( 'Y' ) );
	...
	if( $enviar )
	{
		...
		$query		= "	SELECT		DATE_FORMAT( ven_fecha, '%d-%m-%Y %r' ) AS movimiento,
						...
						AND			( ven_status IN ( 'L', 'S' ) OR ven_total_credito > 0 )
						AND			ven_status != 'R'";
...
<div class="modal-dialog">
	<div class="modal-content">
		...
		<div class="modal-body">
			...
			<div class="row-min text-right h4 text-danger <?= $ocultar ?>">
				<div class="col-md-2">Corte</div>
				<div class="col-md-2">
					<select id="c_dia" class="form-control" <?= $tag_rol ?> >
						<?= $cmb_dias ?>
					</select>
				</div>
				
				<div class="col-md-2">
					<select id="c_mes" class="form-control" <?= $tag_rol ?> >
						<?= $cmb_meses ?>
					</select>
				</div>
				
				<div class="col-md-2">
					<select id="c_anio" class="form-control" <?= $tag_rol ?> >
						<?= $cmb_años ?>
					</select>
				</div>
				
				<div class="col-md-2">Pagar $</div>
				<div class="col-md-2"><input type="text" id="pagar_adeudo" class="form-control text-success" onclick="seleccionar(this)" value="<?= $venta['credito'] ?>" /></div>
			</div>
			...
			<div class="row">
				...
				<div class="col-md-4 text-right">
					<button type="button" onclick="cambiar_status( <?= $id_venta.", ".$folio ?>, 'N' )" class="btn btn-primary btn-sm">Guardar observación</button>
				</div>
			</div>
		...
		<div class="modal-footer">
			...
			<label id="btn_procesar">
				<button type="button" onclick="cambiar_status( <?= $id_venta.", ".$folio ?>, 'S' )" class="btn btn-primary">Entregado</button>
			</label>
		</div>
	</div>
</div>