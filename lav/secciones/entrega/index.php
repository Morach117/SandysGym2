<div class="row">
	<div class="col-md-12">
		<h4 class="text-info"><span class="glyphicon glyphicon-circle-arrow-right"></span> Entrega</h4>
	</div>
</div>
...
<?php
	...
	$b_pago		= request_var( 'b_pago', 1 );
	
	$lista		= obtener_lista_para_entrega( $b_orden, $b_pago, $b_cliente, $b_status );
	$lista_oby	= lista_orden_lavado( $b_orden );
?>

<form action=".?s=entrega" method="post">
	<div class="row">
		<label class="col-md-2">Tipo</label>
		<div class="col-md-10">
			<input type="radio" name="b_status" id="b_status_1" value="L" <?= ( $b_status == 'L' ) ? 'checked':'' ?> >Lavanderia/Edredones <br/>
	...
	<div class="row">
		<label class="col-md-2">Pago</label>
		<div class="col-md-10">
			<input type="radio" name="b_pago" id="b_pago_1" value="1" <?= ( $b_pago == 1 ) ? 'checked':'' ?> >Con pagos pendiente 
			<input type="radio" name="b_pago" id="b_pago_2" value="2" <?= ( $b_pago == 2 ) ? 'checked':'' ?>>Notas pagadas
		</div>
	</div>
