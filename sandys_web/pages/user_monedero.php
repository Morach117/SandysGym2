...
{
    try {
        $query = "SELECT TRIM(CONCAT(IFNULL(s.soc_nombres, ''), ' ', IFNULL(s.soc_apepat, ''), ' ', IFNULL(s.soc_apemat, ''))) AS nombre,
                         GREATEST(IFNULL(s.soc_mon_saldo, 0), IFNULL(p.prep_saldo, 0)) AS saldo,
                         s.soc_id_socio AS id_socio,
                         s.soc_id_consorcio
                  FROM san_socios s
                  LEFT JOIN san_prepago p ON p.prep_id_socio = s.soc_id_socio
                  WHERE s.soc_id_socio = :socioId LIMIT 1";
